export const COUPLE = {
  names: ["Emilola", "Oluwole"],
  eyebrow: "Wedding Programme",
  occasion: "Church",
  venue: "RCCG, House of Triumph",
  date: "Wednesday, 8th July 2026",
};

export const MINISTERS = [
  { name: "Sis. Folasade Adefila", role: "Officiating Minister" },
  { name: "Bro. Opeyemi Omoseebi", role: "Officiating Minister" },
  { name: "A/P Bukola Babalola", role: "Officiating Minister" },
  { name: "Pastor Seun Babalola", role: "Pastor in Charge of Area" },
  { name: "Pastor Alex Musa", role: "Pastor in Charge of Zone 9" },
  { name: "Pastor Bisi Olowoyo", role: "Pastor in Charge of Lagos Province 6" },
  { name: "Pastor Charlse Kpandei", role: "Pastor in Charge of Region 11" },
  { name: "Pastor Sunday Akanbi", role: "Resident Pastor, LFC Irawo" },
];

export const ORDER_OF_SERVICE = [
  "Praise and Worship",
  "Processional Hymn — To God Be the Glory",
  "Opening Prayer",
  "Bible Reading — Gen. 2:18–25",
  "Joining (Exchange of Vows)",
  "Hymn — My Hope Is Built on Nothing Else",
  "Sermon and Prayer for the Couple",
  "Signing of the Marriage Register",
  "Blessing of the Couple",
  "Announcement",
  "Recessional Hymn — Now Thank We All Our God",
  "Benediction",
  "Photographs",
];

export const HYMNS = [
  {
    no: "Hymn 1",
    title: "To God Be the Glory",
    parts: [
      {
        label: "Verse 1",
        lines: [
          "To God be glory, great things He hath done,",
          "So loved He the world that He gave us His Son,",
          "Who yielded His life an atonement for sin",
          "And open'd the life gate that all may go in.",
        ],
      },
      {
        label: "Chorus",
        lines: [
          "Praise the Lord! Praise the Lord!",
          "Let the earth hear His voice!",
          "Praise the Lord! Praise the Lord!",
          "Let the people rejoice!",
          "O come to the Father, through Jesus the Son;",
          "And give Him the glory, great things He hath done!",
        ],
      },
      {
        label: "Verse 2",
        lines: [
          "O perfect redemption, the purchase of blood,",
          "To every believer the promise of God;",
          "The vilest offender who truly believes",
          "That moment from Jesus a pardon receives.",
        ],
      },
      {
        label: "Verse 3",
        lines: [
          "Great things He hath taught us, great things He hath done,",
          "And great our rejoicing through Jesus the Son;",
          "But purer, and higher, and greater will be",
          "Our wonder, our transport, when Jesus we see.",
        ],
      },
    ],
  },
  {
    no: "Hymn 2",
    title: "My Hope Is Built on Nothing Less",
    parts: [
      {
        label: "Verse 1",
        lines: [
          "My hope is built on nothing less",
          "Than Jesus' blood and righteousness;",
          "I dare not trust the sweetest frame,",
          "But wholly lean on Jesus' name.",
        ],
      },
      {
        label: "Chorus",
        lines: [
          "On Christ, the solid Rock, I stand;",
          "All other ground is sinking sand,",
          "All other ground is sinking sand.",
        ],
      },
      {
        label: "Verse 2",
        lines: [
          "When darkness seems to veil His face,",
          "I rest on His unchanging grace;",
          "In every high and stormy gale,",
          "My anchor holds within the veil.",
        ],
      },
      {
        label: "Verse 3",
        lines: [
          "His oath, His covenant, and blood,",
          "Support me in the whelming flood;",
          "When all around my soul gives way,",
          "He then is all my hope and stay.",
        ],
      },
      {
        label: "Verse 4",
        lines: [
          "When He shall come with trumpet sound,",
          "Oh, may I then in Him be found;",
          "Dressed in His righteousness alone,",
          "Faultless to stand before the throne.",
        ],
      },
    ],
  },
  {
    no: "Hymn 3",
    title: "Now Thank We All Our God",
    parts: [
      {
        label: "Verse 1",
        lines: [
          "Now thank we all our God,",
          "With hearts, and hands, and voices,",
          "Who wondrous things hath done,",
          "In whom His world rejoices;",
          "Who, from our mothers' arms,",
          "Hath bless'd us on our way",
          "With countless gifts of love,",
          "And still is ours to-day.",
        ],
      },
      {
        label: "Verse 2",
        lines: [
          "O may this bounteous God",
          "Through all our life be near us,",
          "With ever-joyful hearts",
          "And blessed peace to cheer us,",
          "And keep us in His grace,",
          "And guide us when perplexed,",
          "And free us from all ills",
          "In this world and the next.",
        ],
      },
      {
        label: "Verse 3",
        lines: [
          "All praise and thanks to God",
          "The Father now be given,",
          "The Son and Him who reigns",
          "With Them in highest heaven,",
          "The one, Eternal God,",
          "Whom earth and heaven adore;",
          "For thus it was, is now,",
          "And shall be evermore.",
        ],
      },
    ],
  },
];
